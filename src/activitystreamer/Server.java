...
import activitystreamer.server.Broadcaster.BroadcasterSolution;
...
public class Server
{
	...
	public static void main(String[] args)
	{
		...
		options.addOption("lbp", true, "load balancer port number");
		options.addOption("lbh", true, "load balancer host name");
		...
		if (cmd.hasOption("lbp"))
		{
			try
			{
				int port = Integer.parseInt(cmd.getOptionValue("lbp"));
				Settings.setLoadBalancerPort(port);
			}
			catch (NumberFormatException e)
			{
				log.info("-lbp requires a port number, parsed: " + cmd.getOptionValue("lbp"));
				help(options);
			}
		}
		
		if (cmd.hasOption("lbh"))
		{
			Settings.setLoadBalancerHostname(cmd.getOptionValue("lbh"));
		}
		...
		final BroadcasterSolution bs = BroadcasterSolution.getInstance();
